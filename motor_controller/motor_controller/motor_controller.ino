...
void loop() {
  ...
  if (mbus.receive(0, &msg)) 
  {
    ...
    switch(msg.command) {
      ...
      case COMMAND_PATH_ERROR: {
        Serial3.print("COMMAND_PATH_ERROR");

        float path_error = (msg.params.move_error.error/ 1000.0) - 0.5;
        //float correction  = 10 * path_error;
        float wheel_ratio = 1.0 + 1.0 * path_error;  // 1-> both wheels same speed  
        robot_action->set_wheel_ratio(wheel_ratio);
        //odometer.reset_trip();
        //robot_action->set_wheel_correction(correction);
